...
class Day15Test implements AdventSolutionTest<Integer, Integer> {
  ...
  @ParameterizedTest
  @Part1Source
  void testPart1DoubleHistory(ResourceIdentifier resource, int expected, Day15 day15) {
    assertEquals(expected, day15.run(partialSolve(2020), resource));
  }

  @ParameterizedTest
  @Part2Source
  void testPart2DoubleHistory(ResourceIdentifier resource, int expected, Day15 day15) {
    assertEquals(expected, day15.run(partialSolve(30_000_000), resource));
  }

  private static ThrowingBiFunction<Day15, BufferedReader, Integer> partialSolve(int n) {
    return ((Day15 __, BufferedReader reader) -> Day15.solveForNth(reader, n));
  }
}
